...
def main():
    ...
    transe = TranE(dataset.entity_num, dataset.relation_num, device, dim=embed_dim, d_norm=d_norm, gamma=gamma).to(
        device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (pos, neg) in enumerate(loader):
            ...
            loss.backward()
            optimizer.step()
        print(f"epoch {epoch+1}, loss = {total_loss/dataset.__len__()}")
